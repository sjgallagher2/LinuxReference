...
;; The first part of most s-expressions is a function, followed
;; by input parameters.  
...
;; Note that when using ( the first part of the list
;; MUST be a function, unless you use a macro like '(
...
;; This is a case where a standard macro character has appeared.
;; These include:
;; ---- # (sharpsign)
;; ---- ' (single-quote)
;; ---- : (colon)
;; Technically, ( and ; are also standard macro characters, denoting
;; the start of a list for ( and the start of a comment for ;

;; The single-quote is used for so-called quoting, returning 
;; exactly what you typed instead of evaluating it. 
(defparameter A 1) ; set A => 1
A ; => 1
'A ; => A
(quote A) ; => A     this is the same things as 'A

;; The quasiquote ` is similar to quote, but you can have certain
;; parts of the expression "unquoted" using a comma. 
`(1 2 ,(+ 3 4)) ; => '(1 2 7)

;; Using quasiquote (aka backquote) you can also use the @
;; symbol. This will put the values of that variable into
;; the output list/expression directly.
...
;; The let function is explained in the next section.



#2A((1 2) (3 4)) ; sharpsign-A with a parameter of 2

;; List of all Sharpsign macros (will be covered later):
#\a ; A character 'a' => #\a
#'hello-world ; returns the definition of hello-world => #<FUNCTION HELLO-WORLD...>
#(1 2 3 ) ; creates a vector with 1, 2, and 3 =>#(1 2 3)
#5(1 2 3) ; creates a vector OF LENGTH 5 with given values. => #(1 2 3 3 3)
#*1000111 ; creates a bit vector
#:foo ; an 'uninterned' variable, which is NOT in the workspace
#.foo ; returns an object foo
#B10011 ; a binary number
#O313 ; an octal number
#X5F3 ; a hex number
#4R1221 ; a base-4 number
#C(4 1) ; a complex number 4 + 1i
#2A((1 2) (3 4)) ; an array, requires the '2' before A
#S(foo :bark 4 :pounce 1) ; a structure defined from the struct foo
#P(foo) ; reads an object that is a string
#2=foo  ; reads an object corresponding to foo, labeled as 2
#2#     ; points to an object defined from #2=
#+foo   ; provides a conditional on whether or not FOO exists
#-foo   ; skips the expression following if foo exists
#| Block comments, see above. |# 

;; colon is used for keywords that have a paired up value
(getf(list :a 1 :b 2 :c 3) :a) ; => 1
;; getf and its counter-part setf are described in the next section
...
;;You can print a new line with format and the terpri macro
(format t "~C" #\newline) ; => new line 
(terpri) ; 		    => new line
...
;;Generally, the let function has this syntax:
(let ((var1)
      (var2)
      ...
      )
...
;;With this let you might have to be careful about the expressions you
;; use. The variables are defined in parallel, not sequentially, so
;; the following fails:
(let ((a 'crunch) (b a))
  (write a))
;; => LET: variable A has no value
;; This fails because b is defined with a, but since they are defined in
;; parallel, the variable A has no value

;; To perform all definitions sequentially, use let*
(let* ((a 'crunch) (b a))
  (write a))
;; => CRUNCH

;;To change a variable that you've defined, you have to use the
;; setf command (stands for set field):
(let
  ((letter 'h)
   (num 124)
   (word 'Hello))
...
'(1 2 3) ; => '(1 2 3)
;; Note that these expressions are NOT the same
...
(set-difference '(1 2 3 4) '(4 5 6 7)) ; => (3 2 1), which of the first list are not
				       ; in the second list
...
(multiple-value-bind (a b) (gethash 'd *m*) (list a b))
;;Or write it on several for easier reading
(multiple-value-bind
      (a b)
    (gethash 'd *m*)
  (list a b))
...
;; Note that &key is almost like &optional. It uses keywords 
;; to assign a variable that's been given.
;; Another symbol to use is &rest, which takes the variables
;; that were given, and puts the leftovers in a list.

(defun hello (name &rest from)
    (format () "Hello ~a, from ~a" name from))

(hello "Sam" "Bob" "Bill" "Linda")
; => Hello Sam, from (Bob Bill Linda)
...
;; The symbol &body will show up later as well, for use in 
;; defining macros. It is very similar to &rest.
